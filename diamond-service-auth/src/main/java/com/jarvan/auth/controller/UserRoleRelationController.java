...
public class UserRoleRelationController {
    ...
    @GetMapping(value = "/user-role-relation", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    ...
    @PutMapping(value = "/user-role-relation", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ApiOperation(value = "为某用户授予角色或删除拥有角色", notes = " ")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "successful request"),
            @ApiResponse(code = 400, message = "bad request"),
            @ApiResponse(code = 404, message = "not found"),
    ...
    public ServerResponse<?> authorization(
            @ApiParam(value = "userId", required = true) @RequestParam(value = "userId") Long userId,
            @ApiParam(value = "roleIds") @RequestParam(value = "roleIds",required = false) String roleIds) {
